...
## Come testare il progetto (senza un Mac libero)

Firebase è **già commentato** in `lib/main.dart` e il bootstrap delle
notifiche push è protetto da un `try/catch`: puoi testare da subito
SEGNALA, la mappa, i luoghi salvati e la checklist senza configurare
Firebase. Riabiliterai Firebase quando vorrai testare anche le push.

### Test su Android (telefono vero)
1. Installa Flutter SDK (flutter.dev) e Android Studio sul tuo PC
   Windows/Linux (non serve un Mac per Android).
2. Sul telefono: attiva le Opzioni sviluppatore (Impostazioni > Info
   telefono > tocca 7 volte "Numero build") e il Debug USB.
3. `git clone` del repository, poi `flutter create .` e `flutter pub get`.
4. Metti i font Inter in `assets/fonts/`.
5. Aggiungi in `android/app/src/main/AndroidManifest.xml`, dentro
   `<manifest>` e prima di `<application>`:
   ```xml
   <uses-permission android:name="android.permission.ACCESS_FINE_LOCATION"/>
   <uses-permission android:name="android.permission.ACCESS_COARSE_LOCATION"/>
   ```
6. Collega il telefono via USB, conferma il popup di debug sul telefono,
   poi:
   ```
   flutter run --dart-define=SUPABASE_URL=https://xxxx.supabase.co --dart-define=SUPABASE_ANON_KEY=xxxx
   ```

### Test web (build interamente su GitHub, zero installazioni locali)
Il workflow è già pronto in `.github/workflows/deploy-web.yml`.
1. Metti il progetto su GitHub.
2. Settings > Secrets and variables > Actions: crea i secret
   `SUPABASE_URL` e `SUPABASE_ANON_KEY`.
3. Settings > Pages > Source: "GitHub Actions".
4. Fai un push sul branch `main` (anche solo committando questo README):
   il workflow parte da solo. Quando la tab "Actions" mostra la spunta
   verde, l'URL pubblico compare in Settings > Pages.
5. **Non verificato in questo progetto**: il supporto di MapLibre su
   web. Se la mappa non si carica, apri gli strumenti sviluppatore del
   browser (F12) > Console, e condividi l'errore che vedi.
